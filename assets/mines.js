...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-1dc11eec";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-1dc11eec">'+'<span class="tile-face-1dc11eec tile-front-1dc11eec"></span>'+'<span class="tile-face-1dc11eec tile-back-1dc11eec"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-1dc11eec")){return;}
var back=tile.querySelector(".tile-back-1dc11eec");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-1dc11eec");if(isMine){tile.classList.add("boom-1dc11eec");}
if(dim){tile.classList.add("dimmed-1dc11eec");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-1dc11eec")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-1dc11eec");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-1dc11eec")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-1dc11eec");void panelEl.offsetWidth;panelEl.classList.add("jolt-1dc11eec");}
endRound();}
